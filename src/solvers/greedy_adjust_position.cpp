...
public:
  ...
  SolverOutputs solve(const SolverArguments &args) override {
    ...
    for (size_t i = 0; i < work.size(); ++i) {
      ...
      if (auto vcut = std::dynamic_pointer_cast<VerticalCutInstruction>(ret.solution[i])) {
        ...
        work[i] = vcut;
      } else if (auto hcut = std::dynamic_pointer_cast<HorizontalCutInstruction>(ret.solution[i])) {
        for (int d = -delta; d <= delta; ++d) {
