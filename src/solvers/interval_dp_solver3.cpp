...
public:
  struct Option : public OptionBase {
    ...
    bool allow_point_cut_merge = false;
    ...
    void setOptionParser(CLI::App* app) override {
      ...
      app->add_flag("--interval-dp-3-allow-point-cut-merge", allow_point_cut_merge);
  ...
  SolverOutputs solve(const SolverArguments &args) override {
    ...
    const bool allow_point_cut_merge = getOption<Option>()->allow_point_cut_merge;
    ...
    LOG(INFO) << "allow_point_cut_merge = " << allow_point_cut_merge;
    ...
    int top_level_id = canvas->calcTopLevelId();
    assert(canvas->blocks[std::to_string(top_level_id)]->size == canvas->size());
    ...
    // 各矩形のにかかるコストの係数を計算しておく
    auto multipliers = CreateVector<double>(H, H + 1, W, W + 1, 0.0);
    for (int b = 0; b < H; ++b) {
      for (int t = b + 1; t <= H; ++t) {
        for (int l = 0; l < W; ++l) {
          for (int r = l + 1; r <= W; ++r) {
            multipliers[b][t][l][r] = 1.0 * height * width / ((yticks[t] - yticks[b]) * (xticks[r] - xticks[l]));
          }
        }
      }
    }
...
#pragma omp for schedule(dynamic)
      for (int btlr = 0; btlr < H * (H + 1) * W * (W + 1); ++btlr) {
        ...
        if (b + 1 <= t && t <= H && l + 1 <= r && r <= W) {
          if (multipliers[b][t][l][r] < prune_threshold) continue;  // 面積が大きい領域は、 geometricMedianColor() が重い && 単色でまとめて塗る可能性が低いので枝刈りする
          ...
          color_costs[b][t][l][r] += std::round(ColorInstruction::kBaseCost * multipliers[b][t][l][r]);
        }
      }
    }
      ...
      for (int w = 1; w <= W; ++w) {
        for (int b = 0; b + h <= H; ++b) {
          ...
          for (int l = 0; l + w <= W; ++l) {
            ...
            best_uncolored_divisions[b][t][l][r] = {-1, -1, -1, 0};

            auto& best_cost = best_costs[b][t][l][r];
            best_cost = color_costs[b][t][l][r];
            const int point_cut_cost = std::round(PointCutInstruction::kBaseCost * multipliers[b][t][l][r]);
            const int cut_cost = std::round(VerticalCutInstruction::kBaseCost * multipliers[b][t][l][r]);
            const int color_cost = std::round(ColorInstruction::kBaseCost * multipliers[b][t][l][r]);
            if (allow_point_cut || allow_point_cut_merge) {
              for (int my = b + 1; my < t; ++my) {
                for (int mx = l + 1; mx < r; ++mx) {
                  ...
                  if (allow_point_cut_merge) {
                    { // .0の色に塗ってから point cut し、 .2 と .3 を merge して再帰する
                      const int merge_cost = std::round(MergeInstruction::kBaseCost * std::min(multipliers[my][t][l][mx], multipliers[my][t][mx][r]));
                    ...
                    { // .0の色に塗ってから point cut し、 .1 と .2 を merge して再帰する
                      const int merge_cost = std::round(MergeInstruction::kBaseCost * std::min(multipliers[b][my][mx][r], multipliers[my][t][mx][r]));
                    ...
                    { // .1の色に塗ってから point cut し、 .3 と .0 を merge して再帰する
                      const int merge_cost = std::round(MergeInstruction::kBaseCost * std::min(multipliers[my][t][l][mx], multipliers[b][my][l][mx]));
                    ...
                    { // .3の色に塗ってから point cut し、 .0 と .1 を merge して再帰する
                      const int merge_cost = std::round(MergeInstruction::kBaseCost * std::min(multipliers[b][my][l][mx], multipliers[b][my][mx][r]));
    ...
    stack.emplace_back(0, H, 0, W, std::to_string(top_level_id), false);
    while (!stack.empty()) {
      ...
      if (division[0] >= 0 && division[1] >= 0) {
        assert(allow_point_cut || allow_point_cut_merge);
        const int color_child = division[2];
        const int leave_child = division[3];
        if (color_child == 0) {
          ...
          if (leave_child == 1) {
            ret.solution.push_back(std::make_shared<MergeInstruction>(name + ".2", name + ".3"));
            stack.emplace_back(b, division[0], division[1], r, name + ".1", true);
            stack.emplace_back(division[0], t, l, r, std::to_string(++top_level_id), true);
          } else if (leave_child == 3) {
            ret.solution.push_back(std::make_shared<MergeInstruction>(name + ".1", name + ".2"));
            stack.emplace_back(division[0], t, l, division[1], name + ".3", true);
            stack.emplace_back(b, t, division[1], r, std::to_string(++top_level_id), true);
          } else {
            stack.emplace_back(b, division[0], division[1], r, name + ".1", true);
            stack.emplace_back(division[0], t, l, division[1], name + ".3", true);
            stack.emplace_back(division[0], t, division[1], r, name + ".2", true);
          }
        } else if (color_child == 1) {
          ret.solution.push_back(std::make_shared<ColorInstruction>(name, colors[b][division[0]][division[1]][r]));
          ret.solution.push_back(std::make_shared<PointCutInstruction>(name, Point(xticks[division[1]], yticks[division[0]]))); 
          if (leave_child == 2) {
            ret.solution.push_back(std::make_shared<MergeInstruction>(name + ".3", name + ".0"));
            stack.emplace_back(division[0], t, division[1], r, name + ".2", true);
            stack.emplace_back(b, t, l, division[1], std::to_string(++top_level_id), true);
          } else if (leave_child == 0) {
            ret.solution.push_back(std::make_shared<MergeInstruction>(name + ".2", name + ".3"));
          ...
          if (leave_child == 0) {
            ...
            stack.emplace_back(b, division[0], l, division[1], name + ".0", true);
            stack.emplace_back(b, t, division[1], r, std::to_string(++top_level_id), true);
          } else if (leave_child == 2) {
            ret.solution.push_back(std::make_shared<MergeInstruction>(name + ".0", name + ".1"));
            stack.emplace_back(division[0], t, division[1], r, name + ".2", true);
            stack.emplace_back(b, division[0], l, r, std::to_string(++top_level_id), true);
          } else {
            stack.emplace_back(b, division[0], l, division[1], name + ".0", true);
            stack.emplace_back(b, division[0], division[1], r, name + ".1", true);
            stack.emplace_back(division[0], t, division[1], r, name + ".2", true);
          }
        } else if (color_child == 2) {
